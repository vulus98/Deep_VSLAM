...
class ModelPipeline:
    ...
    def __init__(self, pipeline_steps: List[str], device: torch.device, tag: str, enable_timing: bool = False, training_mode: bool = False,
                 train_outlier_model: bool = False, train_correction_model: bool = False, supervised_training: bool = False):
        available_steps = {
    ...
    def _exec(self, result, batch_size):
        ...
        # if training a network, use ground truth as reference poses for estimation of global position for next estimations
        if(self._train_outlier_model and self.supervised_training): 
            self.pose_matrix_keyframe = result['poses'][-1]
        else:  # if not, just append our estimations to trajectory
