...
from .pose.pose_outlier_network import PoseEstimationOutlierNetwork
...
class ModelPipeline:
    ...
    def __init__(self, pipeline_steps: List[str], device: torch.device, tag: str, enable_timing: bool = False, training_mode: bool = False,
                 train_outlier_model: bool = False, train_correction_model: bool = False, train_pose_outlier_model: bool = False, multiframe_size: int = 5, 
        ...
        available_steps = {
            ...
            'pose-outlier-network': lambda: PoseEstimationOutlierNetwork().to(device),
        ...
        self.pose_outlier_models = []
        self.pose_outlier_model_names = []
        ...
        for step_name in pipeline_steps:
            if not step_name in available_steps: 
                ...
                elif step_name in {'pose-outlier-network'}:
                    self.pose_outlier_models.append(step)
                    self.pose_outlier_model_names.append(step_name)
        ...
        for step, step_name in zip(self._steps, self._step_names):
            if isinstance(step, Module) or isinstance(step, ScriptModule):
                if step_name in {'point-net-outlier-detector'}: #'dlt-pose-solver', 'so3-matrix-projector'
                    ...
                    step.train(self._train_pose_outlier_model)
        return self
    ...
    def _exec(self, result, batch_size):
        ...
        for step, step_name in zip(self._steps, self._step_names):
            ...
            if step_name in {'point-net-outlier-detector', 'dlt-pose-solver', 'so3-matrix-projector'}:
                ...
                with torch.autograd.set_grad_enabled(self.training_mode and self._train_pose_outlier_model):
                    result.update(step(result, self.pose_matrix_keyframe))
            else:
                with torch.autograd.set_grad_enabled(True):
                    if step_name in {'change-keyframe', 'back-to-world-frame'}:
                        result.update(step(result, self.pose_matrix_keyframe,self.pose_matrices_past))
                    else:
                        result.update(step(result))
            if self.enable_timing:
