...
class SelfSupervisedPoseLoss(ScriptModule):
    @configure
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x: Dict[str, torch.Tensor], y: Dict[str, torch.Tensor]) -> torch.Tensor:
        rotation_composition = x['pose_matrix_estimate_opencv'][:, :3, :3].transpose(-1, -2) @ x['pose_matrix_estimate'][:, :3, :3]
        ...
        translation_arg = x['pose_matrix_estimate'][:, :3, 3]-x['pose_matrix_estimate_opencv'][:, :3, 3]
